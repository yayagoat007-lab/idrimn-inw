        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs space-y-5">
          <div className="border-b border-slate-50 pb-3">
            <h3 className="font-extrabold text-sm text-slate-900 uppercase tracking-tight flex items-center gap-1.5">
              ...
              <span>{lang === 'darija' ? "Simulateur d l-Kridi d s-Sna3/Dar" : "Simulateur de Crédit Immobilier"}</span>
            </h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">
              {lang === 'darija' ? "9dr l-khlass dyal chhar dyal d-dar f l-Maghrib" : "Estimez vos mensualités d'après les barèmes marocains"}
            </p>
          </div>
          ...
          <form onSubmit={handleCalculate} className="space-y-4">
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs font-bold text-slate-700 mb-1">
                  <span>{lang === 'darija' ? "Taman d l-bni / d-dar :" : "Prix d'achat du bien :"}</span>
              ...
              <div>
                <div className="flex justify-between text-xs font-bold text-slate-700 mb-1">
                  <span>{lang === 'darija' ? "L-Apport l-khass dyalk :" : "Apport personnel :"}</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    {lang === 'darija' ? "Durée (sanawat)" : "Durée (années)"}
                  </label>
                  <input
                    type="number"
                    min={1}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  {lang === 'darija' ? "Khlass dyal l-foyer n-Nqi f chhar (DH)" : "Revenu net mensuel du foyer (DH)"}
                </label>
                <input
                  type="number"
                  min={1000}
            ...
            >
              {lang === 'darija' ? "Hseb l-khlass dyal chhar" : "Calculer ma mensualité"}
            </button>
          </form>
        </div>
        ...
        <div className="bg-slate-900 text-white border border-slate-800 rounded-3xl p-6 shadow-xs flex flex-col justify-between space-y-6">
          {mortgageResult ? (
            <div className="space-y-6 flex-1 flex flex-col justify-between">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-6">
                  <div className="border-b border-slate-800 pb-3">
                    <p className="text-[10px] text-slate-400 font-bold uppercase">
                      {lang === 'darija' ? "L-Khlass l-Mo9addar f chhar :" : "Mensualité estimée :"}
                    </p>
                  ...
                  <div className="border-b border-slate-800 pb-3">
                    <p className="text-[10px] text-slate-400 font-bold uppercase">
                      {lang === 'darija' ? "Majmou3 l-arbah :" : "Total des intérêts :"}
                    </p>
                  ...
                  <div className="border-b border-slate-800 pb-3">
                    <p className="text-[10px] text-slate-400 font-bold uppercase">
                      {lang === 'darija' ? "Taman l-Kamal d l-kridi :" : "Coût total du crédit :"}
                    </p>
                ...
                {mortgageResult.debtRatio > 45 ? (
                  <div className="bg-rose-500/10 border border-rose-500/20 text-rose-300 rounded-xl p-3 text-[10px] font-semibold mt-4">
                    {lang === 'darija' 
                      ? "⚠️ Ghandek balek : Nisbat l-endettement dyalk kbira 3la 40%. L-bnooq f l-Maghrib (CIH, Attijari, BCP) kaye t-tlbo nisba a9al m 45% bach i-qblo l-kridi dyal d-dar." 
                  ...
                  <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 rounded-xl p-3 text-[10px] font-semibold mt-4">
                    {lang === 'darija' 
                      ? "✓ Nisbat l-endettement mzyana o s-s7iha. L-dossier dyalk 9wi bach t-bda t-hdar m3a l-bnooq dyalk." 
              ...
              <div className="space-y-2">
                <p className="text-[10px] text-slate-400 font-bold uppercase flex items-center gap-1">
                  <BarChart2 size={12} />
                  <span>{lang === 'darija' ? "Jadwal d r-rdad d l-flouss (Mital)" : "Jalons d'amortissement (Exemple)"}</span>
                </p>
                <div className="max-h-24 overflow-y-auto divide-y divide-slate-800 text-[10px] font-bold text-slate-300">
                  {mortgageResult.amortizationTable.slice(0, 5).map((row, i) => (
                    <div key={i} className="py-1 flex justify-between">
                      <span>{lang === 'darija' ? "Chhar" : "Mois"} {row.month}</span>
                      <span>
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center space-y-3 py-12 text-slate-400 font-bold">
              ...
              <p className="text-xs">
                {lang === 'darija' 
                  ? "Kliker bach t-9dr l-khlass d l-financement dyalk." 
                  : "Cliquez sur le bouton pour évaluer vos options de financement."}
              </p>
            </div>
          )}
        </div>
