...
const EVENT_TYPES: { value: MoroccanEventType; label: string }[] = [
  { value: 'ramadan', label: 'Ramadan' },
  { value: 'aid_al_fitr', label: 'Aïd al-Fitr (Sghir)' },
  { value: 'aid_al_adha', label: 'Aïd al-Adha (Kbir)' },
  { value: 'mawlid', label: 'Mawlid al-Nabawi' },
  { value: 'hijri_new_year', label: 'Nouvel an Hijri' },
  { value: 'custom', label: 'Événement Familial / Perso' }
];
...
export function EventForm({ event, onSave, onCancel }: EventFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Strict validation
    if (!name.trim()) {
      setError("Veuillez renseigner le nom de l'événement.");
      return;
    }
    ...
    if (budgetAllocated <= 0) {
      setError("Le budget alloué doit être supérieur à 0 DH.");
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl bg-white border border-slate-200 rounded-2xl p-6 shadow-xl">
      <div className="flex justify-between items-center pb-3 border-b border-slate-100">
        <h3 className="text-sm font-black text-slate-800 tracking-tight flex items-center gap-1.5">
          ...
          <span>{event ? "Modifier l'Événement" : "Ajouter un Événement"}</span>
        </h3>
        <button 
          type="button" 
          onClick={onCancel}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="Ex: Achoura, Mariage de Salma..."
      ...
      <div className="space-y-1.5">
        <label className="block text-[10px] font-black uppercase text-slate-400 tracking-wider">
          Notes d'organisation / Détails
        </label>
        <textarea
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          placeholder="Ex: Achat auprès de l'Adoul, réservation traiteur, liste d'achats..."
          rows={3}
      ...
      <label className="flex items-center gap-2 cursor-pointer pt-1">
        ...
        />
        <span className="text-xs text-slate-600 font-bold">Événement annuel récurrent (Calendrier Hijri/Grégorien estimé)</span>
      </label>
      ...
      <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
        ...
        >
          ...
          <span>{event ? "Modifier l'événement" : "Créer l'événement"}</span>
        </button>
      </div>
    </form>
  );
