...
export function P2PTransferModal({ isOpen, onClose, language }: P2PTransferModalProps) {
  ...
  const t = {
    title: isDarija ? 'Sifet l-flous dghya' : "Transfert d'argent instantané",
    subtitle: isDarija ? 'Envoi P2P sans frais dghya o s-sahl' : 'Envoi P2P instantané sans frais entre portefeuilles Floussi',
    phoneLabel: isDarija ? 'Raqm d l-mousstafid' : 'Numéro du destinataire',
    phonePlaceholder: 'Ex: 0612345678',
    amountLabel: isDarija ? 'L-qadr (DH)' : 'Montant (DH)',
    noteLabel: isDarija ? 'Sbab / Note' : 'Note / Motif (Optionnel)',
    notePlaceholder: isDarija ? 'Ex: Flous lghda' : 'Ex: Déjeuner, Cadeau, etc.',
    balanceLabel: isDarija ? 'Solde dyalek :' : 'Votre solde :',
    nextBtn: isDarija ? 'Zid l-gdam' : 'Suivant',
    pinTitle: isDarija ? 'Dkhel l-PIN dyalek d l-amane' : 'Code PIN de sécurité',
    pinDesc: isDarija ? 'Dkhel 4 d l-arqam dial l-koud' : 'Entrez votre code secret de sécurité à 4 chiffres pour valider le virement',
    pinConfirm: isDarija ? 'Akeked l-mouamala' : 'Confirmer le transfert',
    backBtn: isDarija ? 'Rje3' : 'Retour',
    successTitle: isDarija ? 'T-sifet l-flous b l-khir!' : 'Transfert envoyé avec succès !',
    receiptTitle: isDarija ? 'Reçu d l-intiqal' : 'Reçu de transfert numérique',
    senderLabel: isDarija ? 'Expéditeur' : 'Expéditeur',
    recipientLabel: isDarija ? 'Destinataire' : 'Destinataire',
    transId: isDarija ? 'Raqm d l-mouamala' : 'Numéro de transaction',
    dateLabel: isDarija ? 'Date' : 'Date',
    closeBtn: isDarija ? 'Sedd' : 'Fermer',
    insufficient: isDarija ? 'Ma3ndekch flous kafi' : 'Solde de portefeuille insuffisant'
  };
  ...
  const handleNextStep = () => {
    ...
    if (!phone || phone.length < 9) {
      setError(isDarija ? 'Raqm l-hatif ghalat.' : 'Numéro de téléphone invalide.');
      return;
    }
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      setError(isDarija ? 'Kteb mablagh s7i7.' : 'Veuillez saisir un montant valide supérieur à 0.');
      return;
    }
  ...
  const handleConfirmTransfer = async () => {
    if (pin.length !== 4) {
      setError(isDarija ? 'Kamel l-koud (4 d l-arqam)' : 'Le code PIN doit comporter 4 chiffres.');
      return;
    }
      ...
      <div ref={modalRef} className="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-slate-100 overflow-hidden relative">
        ...
        {step === 'form' && (
          <div className="p-6 space-y-4">
            ...
            <div className="p-3 bg-emerald-50/50 border border-emerald-100 rounded-xl text-[10px] text-emerald-800 font-bold leading-normal">
              🌐 <strong>{isDarija ? "Mode Simulation: " : "Mode Simulation : "}</strong>{isDarija ? "Ma ghay-mchi hta rial hna. Ghi bach t-jreb l-app." : "Aucun argent réel ne sera transféré. Utile pour tester l'UX Floussi."}
            </div>
        ...
        {step === 'pin' && (
          <div className="p-6 space-y-5">
            ...
            >
              {loading ? (isDarija ? 'Khdamin...' : 'Traitement...') : t.pinConfirm}
            </button>
          </div>
        )}
          ...
          <div className="p-6 space-y-5 text-center bg-slate-50/20">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center justify-center gap-1">
                ...
                <span>{isDarija ? "Simulation t-temmat" : "Simulation terminée"}</span>
              </p>
            </div>
            ...
            <div className="bg-white border border-slate-100 rounded-3xl p-5 text-left space-y-4 shadow-sm relative overflow-hidden">
              ...
              <div className="space-y-2.5 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-400 font-bold">{isDarija ? "Sbab" : "Motif"}</span>
              ...
              <div className="pt-3 border-t border-slate-50 flex flex-col items-center gap-2">
                ...
                <span className="text-[8px] uppercase tracking-widest font-black text-slate-400 flex items-center gap-1">
                  <QrCode size={10} />
                  <span>{isDarija ? "Scanni bach t-t'aked" : "Scanner pour vérifier"}</span>
                </span>
              </div>
            </div>
