...
export function TontineInviteModal({ tontineId, tontineName, onClose }: TontineInviteModalProps) {
  ...
  const handleShareWhatsApp = () => {
    const text = `Rejoins ma Jmâa/Daret "${tontineName}" sur l'application Floussi pour cotiser ensemble en toute sécurité ! Lien d'invitation : ${inviteLink}`;
    const url = `https://wa.me/?text=${encodeURIComponent(text)}`;
    window.open(url, '_blank');
  };

  const handleShareSMS = () => {
    const text = `Rejoins ma Jmâa "${tontineName}" sur Floussi : ${inviteLink}`;
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl relative border border-slate-100">
        ...
        <h3 className="text-sm font-black text-slate-800 flex items-center gap-1.5 uppercase tracking-wider mb-2">
          ...
          <span>Inviter à la Jmâa</span>
        </h3>
        <p className="text-[10px] text-slate-400 font-semibold leading-relaxed mb-4">
          Invitez un membre à rejoindre <strong>{tontineName}</strong>. Ils recevront un lien sécurisé pour confirmer leur participation.
        </p>
        ...
        {!showQR ? (
          <div className="space-y-4">
            ...
            <div className="text-[10px] font-bold text-slate-500">
              <label className="block mb-1">Numéro de téléphone (SMS)</label>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 bg-slate-800 hover:bg-slate-900 text-white rounded-xl py-2 text-[10px] font-black uppercase tracking-wide transition-all text-center"
              >
                Envoyer SMS
              </button>
            </div>
            ...
            <button
              ...
              className="w-full py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-100 rounded-xl text-[9px] font-black uppercase text-slate-600 tracking-wider flex items-center justify-center gap-1.5 transition-all"
            >
